...
const uint16_t PROGMEM altctl_combo[] = {KC_RALT,KC_LCTRL,COMBO_END};
...
[JK_COMBO] = COMBO_ACTION(jk_combo),
...
[IO_COMBO] = COMBO_ACTION(io_combo),
[UI_COMBO] = COMBO_ACTION(ui_combo),
};
...
break;
case IO_COMBO:
if (pressed) {SEND_STRING(SS_DOWN(X_LSFT)SS_TAP(X_END)SS_TAP(X_DEL)SS_UP(X_LSFT));}
break;
case UI_COMBO:
if (pressed) {SEND_STRING(SS_DOWN(X_LSFT)SS_TAP(X_HOME)SS_TAP(X_DEL)SS_UP(X_LSFT));}
break;
}
}